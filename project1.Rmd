...
#scooby_gang_colors("Shaggy Green") This was to check if it worked
...
scoobydoo_world_map <- map1 + scale_fill_gradient(name = "Number of Episodes", low = scooby_gang_colors("Mystery Machine Green"), high = scooby_gang_colors("Mystery Machine Blue"), na.value = "gray")+
...
```{r guest_stars}
hex_girls_guest_star <- scoobydoo %>% 
  filter(hex_girls != "FALSE")

batman_guest_star <- scoobydoo %>% 
  filter(batman != "FALSE")

blue_falcon_guest_star <- scoobydoo %>% 
  filter(blue_falcon != "FALSE")

scooby_dum_guest_star <- scoobydoo %>% 
  filter(scooby_dum != "FALSE")

scrappy_doo_guest_star <- scoobydoo %>% 
  filter(scrappy_doo != "FALSE")

#Left Join These
#Also you want to do something with catch phrases
